...
function userRowsFromApi(rows: Array<{ id: string; fullName: string; email: string; status: string; plan: string; dateJoined: string; lastActivityLabel: string; allowedActions: string[] }>, displayIds: Record<string, string>): UserRow[] {
  return rows.map((row) => ({
    id: row.id,
    displayId: displayIds[row.id] ?? "--",
    name: row.fullName,
    email: row.email,
...
function userRowFromDetail(detail: { id: string; initials: string; avatarUrl: string; fullName: string; email: string; status: string; plan: string; dateJoined: string; lastLoginLabel: string; tracmedyPatientId: string | null; phoneNumber: string | null; allowedActions: string[]; billingCycle: string | null; lastPaymentDate: string | null; nextBillingDate: string | null }): UserRow {
  return {
    id: detail.id,
    displayId: detail.tracmedyPatientId ?? "--",
...
function MetricCard({ title, value, trend, icon, tone }: { title: string; value: string; trend: string; icon: React.ReactNode; tone: "blue" | "green" | "red" | "gray" }) {
  ...
  return (
    <article className="admin-panel-shadow rounded-[8px] border border-admin-border bg-white px-5 py-5">
      <div className={["grid h-10 w-10 place-items-center rounded-[10px]", toneClass].join(" ")}>{icon}</div>
      <div className="mt-4 flex items-end justify-between gap-4">
        <div>
          <p className="text-[11px] font-bold uppercase tracking-[0.08em] text-admin-ink/80">{title}</p>
          <p className="mt-2 text-[20px] font-bold tracking-normal text-admin-ink">{value}</p>
        </div>
...
function SubscriptionOverview({ empty, plusUsers, freeUsers, subscriberGrowthPercent }: { empty: boolean; plusUsers: number; freeUsers: number; subscriberGrowthPercent: number }) {
  ...
  return (
    <section className="admin-panel-shadow rounded-[10px] border border-admin-border bg-white px-5 py-6">
      <h3 className="text-[13px] font-bold uppercase tracking-[0.04em] text-admin-ink">Subscription Overview</h3>
      <div className="mt-6 flex flex-col gap-8 lg:flex-row lg:items-center">
        <div className="grid h-[104px] w-[104px] place-items-center rounded-full p-[15px]" style={{ background: empty ? "var(--color-admin-disabled)" : "conic-gradient(var(--color-admin-blue) 0 " + plusPercent + "%, var(--color-admin-disabled) " + plusPercent + "% 100%)" }}>
          <div className="grid h-full w-full place-items-center rounded-full bg-white text-center">
            <div>
              <p className="text-[13px] font-bold text-admin-blue">{empty ? "0" : formatCount(totalSubscribers)}</p>
        ...
        <div className="grid flex-1 gap-8 md:grid-cols-2">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-[12px] font-medium text-admin-muted">Tracmedy Plus</p>
              <p className="text-[13px] font-bold text-admin-ink">{empty ? "0" : formatCount(plusUsers)} <span className="text-[12px] font-medium text-admin-muted">({empty ? 0 : plusPercent}%)</span></p>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <span className="h-3 w-3 rounded-full bg-admin-disabled" />
            <div>
              <p className="text-[12px] font-medium text-admin-muted">Tracmedy Free</p>
              <p className="text-[13px] font-bold text-admin-ink">{empty ? "0" : formatCount(freeUsers)} <span className="text-[12px] font-medium text-admin-muted">({empty ? 0 : freePercent}%)</span></p>
            </div>
          </div>
    ...
    <form action="/users" className="flex flex-col gap-4 px-4 py-4 lg:flex-row lg:items-center lg:justify-between lg:px-4">
      <div className="grid gap-4 md:grid-cols-[326px_120px_120px_150px]">
        <label className="relative">
          ...
          <input name="search" defaultValue={search} className="h-9 w-full rounded-[7px] border border-admin-border bg-white pl-12 pr-3 text-[13px] text-admin-ink outline-none placeholder:text-admin-muted focus:border-admin-blue" placeholder="Search by name, email, or ID..." />
        </label>
        <select name="status" aria-label="Filter by status" defaultValue={status} className="h-9 rounded-[7px] border border-admin-border bg-white px-3 text-[13px] text-admin-muted outline-none focus:border-admin-blue">
        ...
        <select aria-label="Filter by plan" className="h-9 rounded-[7px] border border-admin-border bg-white px-3 text-[13px] text-admin-muted outline-none focus:border-admin-blue">
          <option>All Plans</option>
        </select>
        <button type="button" className="flex h-9 items-center justify-between rounded-[7px] border border-admin-border bg-white px-3 text-[13px] text-admin-muted">
          <span>Date Range</span>
          <CalendarIcon className="h-4 w-4" />
        </button>
      </div>
      <a href={exportHref} className="flex h-9 items-center gap-2 px-3 text-[13px] font-medium text-admin-ink">
...
function UsersTable({ users, totalItems, page, pageSize, totalPages, search, status, exportHref }: { users: UserRow[]; totalItems: number; page: number; pageSize: number; totalPages: number; search: string; status: string; exportHref: string }) {
  ...
  return (
    <section className="bg-white">
      ...
      {empty ? (
        <div className="grid min-h-[410px] place-items-center border-t border-admin-border px-6 text-center">
          <div className="max-w-[370px]">
            ...
            <h3 className="mt-7 text-[18px] font-bold text-admin-ink">No Users Yet</h3>
            <p className="mt-3 text-[13px] leading-5 text-admin-ink/80">
        ...
        <div className="overflow-x-auto px-4">
          <table className="w-full min-w-[980px] border-collapse text-left">
            <thead className="bg-admin-table-head text-[11px] font-bold uppercase tracking-normal text-admin-ink/80">
              <tr>
            ...
            <tbody className="text-[13px] text-admin-ink">
              {users.map((user) => (
                <tr key={user.id} className="border-b border-admin-border/70 last:border-b-0">
                  <td className="px-6 py-4 font-medium text-admin-text/85">{user.displayId}</td>
                  <td className="px-6 py-4">
                    <p className="font-semibold">{user.name}</p>
                    <p className="mt-0.5 text-[12px] text-admin-muted">{user.email}</p>
                  </td>
                  <td className="px-6 py-4"><span className={["rounded-full px-2.5 py-1 text-[11px] font-medium", statusClass(user.status)].join(" ")}>{user.status}</span></td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      <Link href={{ pathname: "/users", query: { user: user.id } }} aria-label={"View " + user.name} className="text-admin-blue"><EyeActionIcon className="h-5 w-5" /></Link>
      ...
      <footer className="flex min-h-[56px] items-center justify-between border-t border-admin-border px-4 text-[12px] text-admin-muted">
  ...
  return (
    <div className="flex min-h-[68px] items-center gap-4 border-b border-admin-border px-4 last:border-b-0">
      ...
      <div>
        ...
        <p className="mt-1 text-[13px] font-medium text-admin-text">{value}</p>
      </div>
    </div>
  );
}
...
function UserProfileDrawer({ user }: { user: UserRow }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-admin-overlay backdrop-blur-[3px]">
      <aside className="ml-auto flex h-dvh w-full max-w-[560px] flex-col bg-white shadow-2xl">
        <header className="flex h-[88px] items-center justify-between border-b border-admin-border px-6">
          <div className="flex items-center gap-4">
            <div className="grid h-12 w-12 place-items-center rounded-full bg-admin-blue text-[16px] font-bold text-white">{initials}</div>
            <div>
              <h2 className="text-[18px] font-bold text-admin-ink">{user.name}</h2>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-6 py-6">
          <section>
            ...
            <div className="mt-3 overflow-hidden rounded-[8px] border border-admin-border">
              <DetailRow icon={<IdIcon className="h-5 w-5" />} label="User ID" value={user.displayId} />
        ...
        <footer className="sticky bottom-0 z-10 grid grid-cols-2 gap-3 border-t border-admin-border bg-white px-6 py-4">
          ...
          {drawerAction ? (
            <form action={changeUserStatusAction}>
              ...
              <button type="submit" className={["flex h-10 w-full items-center justify-center gap-2 whitespace-nowrap rounded-[8px] border text-[13px] font-bold text-white", drawerButtonClass].join(" ")}>{drawerAction === "suspend" ? <BanIcon className="h-5 w-5" /> : <UserCheckIcon className="h-5 w-5" />}{drawerAction === "suspend" ? "Suspend User" : "Activate User"}</button>
            </form>
          ) : null}
        </footer>
      </aside>
...
export default async function UsersPage({ searchParams }: UsersPageProps) {
  ...
  if (usersList) {
    const details = await Promise.all(usersList.items.map((row) => getAdminUserDetail(token, row.id)));
    usersList.items.forEach((row, index) => { displayIds[row.id] = details[index]?.tracmedyPatientId ?? "--"; });
  }
  const showingProfile = Boolean(params.user);
  const empty = params.state === "empty";
  const users = empty ? [] : usersList ? userRowsFromApi(usersList.items, displayIds) : [];
    ...
    <>
      <main className="px-6 py-7 lg:px-8">
        <div className="flex items-start justify-between gap-6">
          <div>
            <h2 className="text-[20px] font-bold tracking-normal text-admin-ink">Users</h2>
            <p className="mt-2 text-[13px] font-medium text-admin-muted">Manage all registered users</p>
          </div>
          <a href={exportHref} className="flex h-10 items-center gap-2 rounded-[8px] bg-admin-blue px-4 text-[13px] font-bold text-white">
            <DownloadIcon className="h-5 w-5" />
            Export
          </a>
        </div>
