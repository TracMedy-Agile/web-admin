...
export async function GET(request: NextRequest) {
  ...
  const url = backendUrl() + "/admin/facilities/export" + (query ? "?" + query : "");
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Unable to reach facilities export service.";
    return apiError({ message }, 502);
  }

  if (!response.ok) {
    return apiError(await readJson(response), response.status, "Unable to export facilities.");
  }
  ...
  return new NextResponse(csv, {
    headers: {
      ...
      "Content-Disposition": 'attachment; filename="tracmedy-facilities.csv"',
    },
  });
}